...
void operations(const string &filename, bool fileformat) {
  ...
  while (true) {
    ...
    std::cout << "1 - Generate folder structure for specific board id & "
                 "starting address to use it in the Python Histogram Script "
                 "(not yet implemented)"
              << std::endl
              << std::flush;
    std::cout << "2 - Generate folder structure to use it in the Python "
                 "Histogram Script"
              << endl
              << flush;
    std::cout << "3 - Generate averaged images for specific board id & "
                 "starting address"
              << endl
              << flush;
    std::cout
        << "4 - Generate all averaged images per board id & starting address"
        << endl
        << flush;
    std::cout << "5 - Generate all entropy files" << endl << flush;
    std::cout
        << "6 - Generate all probabilities per board id & starting address"
        << endl
        << flush;
    ...
    switch (actionNumber) {
      case 1:  // specific folder structure
        inputBoardID =
            readInputFromConsole("Please enter Board ID (or \"all\"");
        inputAddress =
            readInputFromConsole("Please enter Memory Address (or \"all\"");
        // TODO
        break;
      ...
      case 7:  // exit
        ...
        return;
      default:
        std::cout << "No right number was entered, please try again" << endl
                  << flush;
    }
...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    ...
    std::cout << "Usage: " << argv[0]
              << "-f {CSV filename} -i {request or alternative}" << std::endl;
    std::cout
        << "[request] ist equal to a file that was requested on the github page"
        << std::endl;
    std::cout << "[alternative] ist equal to a file that was provided via email"
              << std::endl;
  }
  ...
  while ((opt = getopt(argc, argv, "f:i:")) != -1) {
    switch (opt) {
      ...
      default: /* '?' */
        fprintf(stderr,
                "Usage: %s -f {CSV filename} -i {request or alternative}\n",
                argv[0]);
        exit(-1);
    }
  }
